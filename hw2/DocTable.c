...
DocID_t DocTable_Add(DocTable *table, char *docname) {
  ...
  HTKey_t key = FNVHash64((unsigned char *) doccopy, strlen(doccopy));

  if (HashTable_Find(table->docname_to_docid, key, &oldkv)) {
      free(docid);
      free(doccopy);
...
char *DocTable_GetDocName(DocTable *table, DocID_t docid) {
  ...
  bool result = HashTable_Find(table->docid_to_docname, docid, &kv);
  if (result) {
