...
static int MI_SearchResultComparator(LLPayload_t e1, LLPayload_t e2) {
    SearchResult *search1 = (SearchResult *) e1;
    SearchResult *search2 = (SearchResult *) e2;

    if (search1->rank > search2->rank) {
        return 1;
    } else if (search1->rank < search2->rank) {
        return -1;
    } else {
        return 0;
    }
...
void MemIndex_AddPostingList(MemIndex *index, char *word, DocID_t docid,
    ...
    if (!HashTable_Find(index, key, &mi_kv)) {
        ...
        //   (1) find existing memory or allocate new memory for the
        //   WordPostings'
        ...
        //       mapping.
        //   (3) insert the the new WordPostings into the inverted index
        //   (ie, into
        //       the "index" table).
        // 1
        ...
        wp->word = word;
        // 2
        wp->postings = HashTable_Allocate(23);
        // 3
        mi_kv.key = key;
        mi_kv.value = wp;
        ...
        // Ensure we don't have hash collisions
        // (two different words that hash to
    ...
    HTKeyValue_t newkv;
    newkv.key = docid;
    newkv.value = (void *)postings;
    HashTable_Insert(wp->postings, newkv, &postings_kv);
}
...
LinkedList* MemIndex_Search(MemIndex *index, char *query[], int querylen) {
    ...
    // appears in that document).  Finally, append the SearchResult onto
    // retline.
    ...
    bool next = true;
    while (HTIterator_IsValid(iter) && next) {
        search = malloc(sizeof(SearchResult));
        Verify333(search != NULL);
        HTIterator_Get(iter, &kv);
        search->docid = kv.key;
        search->rank = LinkedList_NumElements(kv.value);
        LinkedList_Append(retlist, (void *)search);
        next = HTIterator_Next(iter);
    }
