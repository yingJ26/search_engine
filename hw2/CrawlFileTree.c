...
static void HandleDir(char *dirpath, DIR *d, DocTable **doctable,
  ...
  for (i = 0, dirent = readdir(d); true; i++, dirent = readdir(d)) {  // you
      // probably want to change/add to this loop
      if (dirent == NULL) {
          break;
      }
...
static void HandleFile(char *fpath, DocTable **doctable, MemIndex **index) {
  ...
  // of the file.
  char* filecontent = ReadFileToString(fpath, &filelen);
  tab = ParseIntoWordPositionsTable(filecontent);
  if (tab == NULL) {
      return;
  }
