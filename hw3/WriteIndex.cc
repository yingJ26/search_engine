...
int WriteIndex(MemIndex *mi, DocTable *dt, const char *filename) {
  ...
  int hdbytes = WriteHeader(f, dtbytes, mibytes);
  if (hdbytes == kFailedWrite) {
    fclose(f);
    unlink(filename);
    return kFailedWrite;
  }
...
static int WriteHeader(FILE *f,
  ...
  CRC32 crc;
  char nextByte;
  // right location -> beginning of doctable
  if (fseek(f, sizeof(IndexFileHeader), SEEK_SET) != 0) {
      return kFailedWrite;
  }
  int len = doctableBytes + memidxBytes;
  for( int i = 0; i < len; i++) {
      if (fread(&nextByte, sizeof(char), 1, f) != 1) {
        return kFailedWrite;
      }
      crc.FoldByteIntoCRC((uint8_t)nextByte);
  }
...
static int WriteHashTable(FILE *f,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int numElements = LinkedList_NumElements(bucket);

    // write bucket record
    if (WriteHTBucketRecord(f, recordPos, numElements,
                            bucketPos) == kFailedWrite) {
      return kFailedWrite;
    }
    recordPos += sizeof(BucketRecord);

    // write bucket content iff chain is not empty
    if (numElements != 0) {
      res = WriteHTBucket(f, bucketPos, bucket, fn);
      if (res == kFailedWrite) {
        return kFailedWrite;
      }
    }
...
static int WriteHTBucketRecord(FILE *f,
  ...
  BucketRecord bucketRecord(numElts, bucketOffset);
  bucketRecord.toDiskFormat();
  ...
  if (fwrite(&bucketRecord, sizeof(BucketRecord), 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteHTBucket(FILE *f,
  ...
  for (int i = 0; i < numElts; i++) {
    ...
    ElementPositionRecord elementPositionRecord(elementPos);
    elementPositionRecord.toDiskFormat();
    if (fwrite(&elementPositionRecord, sizeof(ElementPositionRecord), 1, f) !=
    1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
...
static int WriteDocidToDocnameFn(FILE *f,
  ...
  int16_t filenameBytes = strlen(filename);  // you may want to change this
  ...
  int data = sizeof(DoctableElementHeader) + filenameBytes;
  return data;
}
...
static int WriteDocidToPositionListFn(FILE *f,
  ...
  DocIDElementHeader docIDHeader(docID, numPositions);
  docIDHeader.toDiskFormat();

  if (fseek(f, offset, SEEK_SET) != 0) {
    return kFailedWrite;
  }

  if (fwrite(&docIDHeader, sizeof(DocIDElementHeader), 1, f) != 1) {
    return kFailedWrite;
  }
  ...
  return sizeof(DocIDElementHeader)
    + numPositions * sizeof(DocIDElementPosition);
}
...
static int WriteWordToPostingsFn(FILE *f,
  ...
  uint16_t wordBytes = strlen(wp->word);
  ...
  // place in the file.
  if (fwrite(wp->word, 1, wordBytes, f) != wordBytes) {
    return kFailedWrite;
  }
