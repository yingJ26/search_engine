...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
      folders.push_back(string(argv[i]));
  }
  QueryProcessor qp(folders);

  // STEP 1:
  // Implement filesearchshell!
  while (1) {
      // prints to the console a prompt telling the user to
      // input the next query
      cout << "Enter input:" << endl;
      string line;
      // reads a white-space separated list of query words from std::cin
      getline(cin, line);
      // cmd + D
      if (cin.eof()) {
          break;
      ...
      while (iss >> word) {
          ...
          // construct a vector of c++ strings out of them.
          query.push_back(word);
      }
      // print the query results to std::cout
      auto res = qp.ProcessQuery(query);
      if (res.size() == 0) {
          cout << "  [no results]" << endl;
      } else {
          for (auto it : res) {
              cout << "  " << it.documentName<< " (" << it.rank << ")" << endl;
          }
      }
  }
