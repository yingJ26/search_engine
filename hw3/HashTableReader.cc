...
HashTableReader::LookupElementPositions(HTKey_t hashKey) {
  ...
  int len = bucketRec.chainNumElements;
  for (int i = 0; i < len; i++) {
    ElementPositionRecord elementPositionRecord;

    Verify333(fread(&elementPositionRecord, sizeof(ElementPositionRecord),
            1, file_) == 1);
    elementPositionRecord.toHostFormat();
    // append to the end of the list
    retval.push_back(elementPositionRecord.position);
  }
