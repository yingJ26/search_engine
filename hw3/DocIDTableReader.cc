...
bool DocIDTableReader::LookupDocID(const DocID_t &docID,
  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    if (currHeader.docID == docID) {
      ...
      DocIDElementPosition docIDpos;
      for (int i = 0; i < currHeader.numPositions; i++) {
        Verify333(fread(&docIDpos, sizeof(DocIDElementPosition),
                  1, file_) == 1);
        docIDpos.toHostFormat();
        retval->push_back(docIDpos.position);  // append to the end of list
      }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() {
  ...
  for (int i = 0; i < header_.numBuckets; i++) {
    ...
    // the index file.
    int64_t offset = offset_ + sizeof(BucketListHeader) + i*sizeof
        (BucketRecord);
