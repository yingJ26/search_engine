...
// Use the iterator of the chain to check if the chain contains the key
// if we can find the key, its corresponding keyvaluePair will be
// returned to **keyvaluePair
static bool ContainsKey(LLIterator *iter,
                        HTKey_t key, HTKeyValue_t** keyvaluePair);
...
void HashTable_Free(HashTable *table,
    ...
    for (i = 0; i < table->num_buckets; i++) {
        ...
        // value_free_function -- which takes a HTValue_t,
        // not an LLPayload_t -- to
...
bool HashTable_Insert(HashTable *table,
    ...
    bool result = false;
    HTKeyValue_t *keyvaluePair;
    // return the oldkeyvalue and remove it if we can find it
    if (ContainsKey(iter, newkeyvalue.key, &keyvaluePair)) {
        result = true;
        *oldkeyvalue = *keyvaluePair;
        // immediately free it after using it
        free(keyvaluePair);
        LLIterator_Remove(iter, LLNoOpFree);
        table->num_elements -=1;
    }
    // update the value anyways
    HTKeyValue_t *newkeyvaluePair = (HTKeyValue_t *)
            malloc(sizeof(HTKeyValue_t));
    Verify333(newkeyvaluePair != NULL);
    *newkeyvaluePair = newkeyvalue;
    // push it into the chain
    LinkedList_Push(chain, (void *)newkeyvaluePair);
...
bool HashTable_Find(HashTable *table,
    ...
    HTKeyValue_t *keyvaluePair;
    // if we can find it
    if (ContainsKey(iter, key, &keyvaluePair)) {
        *keyvalue = *keyvaluePair;
        // free the iterator before returns
        LLIterator_Free(iter);
        return true;
    }
    LLIterator_Free(iter);
...
bool HTIterator_IsValid(HTIterator *iter) {
    ...
    return (iter->bucket_it != NULL && LLIterator_IsValid(iter->bucket_it)
            && iter->ht != NULL && iter->ht->num_elements != 0 &&
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
    ...
    if (!HTIterator_IsValid(iter) || iter->ht == NULL
        || iter->ht->num_elements == 0) {
        return false;
    }
    HTKeyValue_t *oldkeyvalue;
    LLIterator_Get(iter->bucket_it, (void *)&oldkeyvalue);
...
static bool ContainsKey(LLIterator *iter, HTKey_t key, HTKeyValue_t**
keyvaluePair) {
    Verify333(iter != NULL);
    ...
    do {
        LLIterator_Get(iter, (void *)keyvaluePair);
